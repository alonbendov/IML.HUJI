...
class MultivariateGaussian:
    ...
    def fit(self, X: np.ndarray) -> MultivariateGaussian:
        ...
        matrices = np.array(
            [np.outer((X[i] - self.mu_), (X[i] - self.mu_)) for i
             in range(n_samples)])
        self.cov_ = (1 / (n_samples - 1)) * matrices.sum(axis=0)
    ...
    def pdf(self, X: np.ndarray):
        ...
        n_samples, n_features = X.shape
        coef = 1 / np.sqrt(
            np.power(np.pi * 2, n_features) * np.linalg.det(self.cov_))
        exp_arg = np.array([
            -0.5 * np.transpose(X[i] - self.mu_) @ np.linalg.inv(self.cov_) @ (
                    X[i] - self.mu_) for i in range(n_samples)])
    ...
    def log_likelihood(mu: np.ndarray, cov: np.ndarray,
        ...
        arg = -0.5 * np.apply_along_axis(calc_exp_arg, 1, X).sum()
        coef_log = np.log(1 / np.sqrt(
